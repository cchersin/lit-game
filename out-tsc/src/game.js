...
export class Game {
    ...
    static fromJSON(json) {
        ...
        g.whiteDeck = json.whiteDeck.map((c) => Card.fromJSON(c));
        g.blackDeck = json.blackDeck.map((c) => Card.fromJSON(c));
        g.blackCard = Card.fromJSON(json.blackCard);
        g.tableCards = json.tableCards.map((c) => Card.fromJSON(c));
        g.players = json.players.map((p) => Player.fromJSON(p));
        g.rounds = json.rounds.map((r) => Round.fromJSON(r));
        return g;
    }
}
